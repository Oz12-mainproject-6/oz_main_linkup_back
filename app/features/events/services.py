...
class EventService:
        ...
        try:
            ...
            for index, row in df.iterrows():
                try:
                    ...
                    end_time = pd.to_datetime(row['end_time']) if pd.notna(row.get('end_time')) else None
                    ...
                    event_data = {
                        ...
                        'visibility': EventVisibility(row.get('visibility', EventVisibility.PUBLIC))
                    }
            ...
            logger.info(f"File upload processed: {created_count}/{len(df)} events created")
    ...
    async def generate_template() -> BinaryIO:
        ...
        # 헤더 설정
        headers = [
            'artist_id', 'title', 'description', 'start_time',
            'end_time', 'location', 'category', 'visibility'
        ]
        ...
        info_data = [
            ...
            ["start_time", "시작 시간 (YYYY-MM-DD HH:MM:SS)", "Yes", "2024-12-25 19:00:00"],
            ["end_time", "종료 시간 (YYYY-MM-DD HH:MM:SS)", "No", "2024-12-25 22:00:00"],
        ...
        # 샘플 데이터 (메인 시트에)
        sample_data = [
            [1, "Sample Concert", "Sample Description", "2024-12-25 19:00:00",
             "2024-12-25 22:00:00", "Seoul Olympic Park", "concert", "public"],
            [1, "Fan Meeting", "Meet & Greet Event", "2024-12-30 15:00:00",
             "2024-12-30 17:00:00", "Coex Hall", "fanmeeting", "subscribers_only"]
        ]
        ...
        for sheet in [ws, ws_info]:
            for column in sheet.columns:
                ...
                sheet.column_dimensions[column[0].column_letter].width = min(max_length + 2, 25)
    ...
    async def trigger_notifications():
        ...
        try:
            ...
            if not Tortoise.get_connection("default"):
                logger.warning("Database connection not available, skipping notifications")
                return
            ...
            for event in instant_events:
                try:
                    ...
                    logger.error(f"Failed to send instant notification for event {event.id}: {str(e)}")
            ...
            for event in upcoming_events:
                try:
                    ...
                    logger.error(f"Failed to send upcoming notification for event {event.id}: {str(e)}")
