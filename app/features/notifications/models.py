...
class Notifications(TimestampMixin):
    ...
    user = fields.ForeignKeyField("app.features.users.models.User", related_name="notifications", description="사용자")
    ...
    entity_type = fields.CharField(max_length=50, null=True, description="관련 엔티티 타입")
...
class Subscription(TimestampMixin):
    ...
    user = fields.ForeignKeyField("app.features.users.models.User", related_name="subscriptions", description="구독자") 
    artist = fields.ForeignKeyField("app.features.artists.models.Artist", related_name="subscribers", description="아티스트")
    
    class Meta:
        table = "subscription"
        unique_together = [("user", "artist")]  # 같은 사용자가 같은 아티스트를 중복 구독 방지