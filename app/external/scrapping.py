...
def get_artist_schedule(artist_name: str, unit_id: str):
    ...
    for script in scripts:
        if script.string and ('schedule' in script.string.lower() or 'event' in script.string.lower()):
            print("📜 스케줄 관련 스크립트 발견")
            # JSON 패턴 찾기
            json_matches = re.findall(r'schedules?\s*:\s*(\[.*?\])', script.string, re.DOTALL)
            if json_matches:
...
def extract_type_info(element):
    ...
    # 클래스에서 타입 유추
    classes = element.get('class', [])
    type_keywords = ['broadcast', '방송', 'concert', '콘서트', 'fanmeeting', '팬미팅', 'release', '발매']
